...
def search_transactions():
    if request.method == 'POST':
        ...
        filtered_transactions = list(filter(lambda x: x['amount']>=minimum and x['amount']<=maximum,transactions))
...
@app.route("/balance")
def total_balance():
    return "Total Balance :" + str(sum([transaction['amount'] for transaction in transactions]))
